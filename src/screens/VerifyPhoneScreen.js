...
export default function VerifyPhoneScreen({ navigation }) {
  ...
  const sendVerificationCode = async () => {
    if (!profile?.phone) {
      Alert.alert('오류', '핸드폰 번호 정보가 없습니다.');
      return;
    }
    ...
    try {
      ...
      // TODO: 실제로는 SMS 발송 서비스 사용 (Twilio, AWS SNS 등)
      // 지금은 콘솔에 코드 출력
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      // 이메일 + 핸드폰 모두 인증 완료 시 "실존 인증" 뱃지 부여
      ...
      const hasEmailVerified = verifications?.some(v => v.kind === 'email');
      const hasPhoneVerified = verifications?.some(v => v.kind === 'phone');
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>핸드폰 인증</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>핸드폰 인증</Text>
        <Text style={styles.description}>
          가입하신 핸드폰 번호로 인증 코드를 발송합니다.{'\n'}
          실명과 일치하는 경우에만 인증이 완료됩니다.
        </Text>

        <View style={styles.infoBox}>
          <Text style={styles.infoLabel}>인증 핸드폰</Text>
          <Text style={styles.infoValue}>{profile?.phone || '핸드폰 번호 없음'}</Text>
        </View>
        ...
        <View style={styles.noticeBox}>
          ...
          <Text style={styles.noticeText}>
            인증 코드는 5분간 유효합니다. SMS를 받지 못하셨다면 번호를 확인해주세요.
          </Text>
        </View>
      </View>
    </View>
